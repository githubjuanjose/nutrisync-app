...
export function GoalOrb({ index, label, size = 200 }: { index: number; label: string; size?: number }) {
  ...
  return (
    <View style={[styles.orb, { width: size, height: size, borderRadius: size / 2 }]}>
      ...
      {/* R6-f19: long ES labels shrink instead of orphaning a letter */}
      <Text style={[styles.txt, { paddingHorizontal: size * 0.11, fontSize: size * 0.085 }]} numberOfLines={4} adjustsFontSizeToFit>{label}</Text>
    </View>
  );
}
